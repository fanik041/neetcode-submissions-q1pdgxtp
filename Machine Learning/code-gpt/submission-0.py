...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.transformer_blocks = nn.Sequential(*[
            GPT.TransformerBlock(model_dim, num_heads)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        # 4. Apply softmax(dim=-1) and round to 4 decimal places
        batch_size, seq_len = context.shape
        positions = torch.arange(seq_len, device=context.device)

        token_emb = self.word_embeddings(context)
        pos_emb = self.position_embeddings(positions)
        embedded = token_emb + pos_emb

        embedded = self.transformer_blocks(embedded)
        embedded = self.final_layer_norm(embedded)
        logits = self.vocabulary_projection(embedded)
